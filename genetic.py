...
def _eaFunction(population, toolbox, cxpb, mutpb, ngen, ngen_no_change=None, stats=None,
    ...
    for gen in range(1, ngen + 1):
        ...
        log_list.append(np.array(halloffame, dtype=np.bool)[0])
    ...
    return population, logbook, log_list
...
class GeneticSelectionCV(BaseEstimator, MetaEstimatorMixin, SelectorMixin):
    ...
    def _fit(self, X, y):
        ...
        _, log, self.log_list = _eaFunction(pop, toolbox, cxpb=self.crossover_proba, mutpb=self.mutation_proba,
    ...
    def analysis(self):
        hist = [0 for i in range(30)]
        for i in self.log_list:
            hist = hist + i
        hist = np.array(hist)
        shist = np.argsort(-hist)
        return hist, shist, self.support_